...
from chroma_feedback import consumers, logger, wording
...
def load_consumer(consumer_name : str) -> ModuleType:
	try:
		return importlib.import_module(consumers.ALL[consumer_name])
	except ModuleNotFoundError:
		logger.error(wording.get('consumer_not_found').format(consumers.ALL[consumer_name]) + wording.get('exclamation_mark'))
		sys.exit()

...
def init(program : ArgumentParser) -> None:
	...
	if not ARGS:
		program.add_argument('--plantronics-status-indicator-light-id', action = 'append')
...
def run(producer_report : List[ProducerReport]) -> List[Consumer]:
	lights = filter_lights(get_lights(), ARGS.get('plantronics_status_indicator_light_id'))

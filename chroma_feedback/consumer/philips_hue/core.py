...
def init(program : ArgumentParser) -> None:
	...
	if not ARGS:
		...
		program.add_argument('--philips-hue-light-name', action = 'append')
...
def run(producer_report : List[ProducerReport]) -> List[Consumer]:
	api = get_api(ARGS.philips_hue_bridge_ip)

	# use groups

	if ARGS.philips_hue_group_name:
		groups = filter_groups(api.get_group(), ARGS.philips_hue_group_name)
	...
	lights = filter_lights(api.get_light_objects(), ARGS.philips_hue_light_name)
...
def discover_bridge_ips() -> List[str]:
	...
	try:
		...
		logger.warn(wording.get('ip_not_found').format('PHILIPS HUE BRIDGE') + wording.get('exclamation_mark'))
	return bridge_ips

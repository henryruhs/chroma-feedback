	...
	for group in groups:
		...
		if status == 'passed':
			result.append(
			{
				...
				'type': 'group',
				'name': group_name,
				'active': static(group, color.get_passed_hue()),
				'status': status
			})
		if status == 'process':
			result.append(
			{
				...
				'type': 'group',
				'name': group_name,
				'active': static(group, color.get_process_hue()),
				'status': status
			})
		if status == 'errored':
			result.append(
			{
				...
				'type': 'group',
				'name': group_name,
				'active': static(group, color.get_errored_hue()),
				'status': status
			})
		if status == 'failed':
			result.append(
			{
				...
				'type': 'group',
				'name': group_name,
				'active': static(group, color.get_failed_hue()),
				'status': status
			})
	return result
